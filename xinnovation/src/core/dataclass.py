...
def tensor_to_trajectory(traj_params: torch.Tensor, traj_id: int = 0, t0: float = 0.0) -> ObstacleTrajectory:
    ...
    return ObstacleTrajectory(
        ...
        motion=motion,
        yaw=yaw,
        length=float(np.exp(traj_params[TrajParamIndex.LENGTH])),
        width=float(np.exp(traj_params[TrajParamIndex.WIDTH])),
        height=float(np.exp(traj_params[TrajParamIndex.HEIGHT])),
