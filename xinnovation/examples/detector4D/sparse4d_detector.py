...
class Sparse4DDetector(nn.Module):
    ...
    def forward(self, batch: Dict) -> List[Dict[str, torch.Tensor]]:
        ...
        init_tgts = self.query_embed.repeat(B, 1, 1)
        init_pos_embeds = self.anchor_encoder(init_trajs)
        
        trajs, quality = self._decode_trajs(batch, init_trajs, init_pos_embeds, init_tgts, all_features_dict, enable_self_attn=True)
        ct_trajs, ct_quality = self._decode_trajs(batch, init_trajs, init_pos_embeds, init_tgts, all_features_dict, enable_self_attn=False)  # cross-attention branch of DAC-DETR
